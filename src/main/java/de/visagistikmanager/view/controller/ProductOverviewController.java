...
public class ProductOverviewController implements Initializable {
		...
		productTable.setRowFactory(tv -> {
			...
			row.setOnMouseClicked(event -> {
				if (event.getClickCount() == 2 && !row.isEmpty()) {
					final Product selectedCustomer = row.getItem();
	...
	private void buildCustomerView(final Product selectedCustomer) {
		...
		// e.printStackTrace();
		// }
		// final CustomerViewController customerViewController =
		// customerViewLoader.getController();
